...
interface Props {
  data: { label: string; profit: number; count: number }[];
  accentColor?: string;
...
const StrategyPerformanceChart = ({ data, accentColor, medianLineColor, medianLineVisible, usePercent }: Props) => {
  const uid = useId();
  ...
  if (data.every(d => d.count === 0)) {
    return (
      <div className="h-full flex items-center justify-center text-muted-foreground text-sm">
        Ajoutez des trades pour voir l'impact du respect de la stratégie
      </div>
    );
  }
    ...
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={dataAvg} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
        <defs>
          ...
          <linearGradient id={`stratProfit-${uid}`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={profitColor} stopOpacity={1} />
            <stop offset="100%" stopColor={profitColor} stopOpacity={0.4} />
          </linearGradient>
          <linearGradient id={`stratLoss-${uid}`} x1="0" y1="0" x2="0" y2="1">
        ...
        <XAxis dataKey="label" tick={{ fontSize: 11, fill: 'hsla(215, 15%, 55%, 0.7)' }} tickLine={false} axisLine={false} />
        ...
        <Bar dataKey="profit" radius={[6, 6, 0, 0]} barSize={26}>
          {data.map((entry, index) => (
            <Cell
              key={index}
              fill={entry.profit >= 0 ? `url(#stratProfit-${uid})` : `url(#stratLoss-${uid})`}
