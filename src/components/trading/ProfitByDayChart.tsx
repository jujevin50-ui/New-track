...
interface Props {
  data: { day: string; profit: number; count: number }[];
  accentColor?: string;
...
const ProfitByDayChart = ({ data, accentColor, medianLineColor, medianLineVisible, usePercent }: Props) => {
  const uid = useId();
  ...
  if (data.every(d => d.count === 0)) {
    return (
      <div className="h-full flex items-center justify-center text-muted-foreground text-sm">
        Ajoutez des trades pour voir le profit par jour
      </div>
    );
  }
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={dataAvg} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
        ...
        <XAxis dataKey="day" tick={{ fontSize: 11, fill: 'hsla(215, 15%, 55%, 0.7)' }} tickLine={false} axisLine={false} />
        ...
        <Bar dataKey="profit" radius={[6, 6, 0, 0]} barSize={16}>
          {data.map((entry, index) => (
            <Cell
              key={index}
              fill={entry.profit >= 0 ? `url(#dayProfit-${uid})` : `url(#dayLoss-${uid})`}
...
export default ProfitByDayChart;
