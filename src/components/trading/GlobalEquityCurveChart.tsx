...
const GlobalEquityCurveChart = ({ data, accentColor, liveColor, medianLineColor, medianLineVisible }: Props) => {
  const uid = useId();
  ...
  if (data.length === 0) {
    return (
      <div className="h-full flex items-center justify-center text-muted-foreground text-sm">
        Add trades to see the global equity curve
      </div>
    );
  }

  const values = data.map(d => d.balance);
  const initialBalance = values[0] || 0;
  const max = Math.max(...values);
  const padding = (max - initialBalance) * 0.1 || max * 0.02;
  const domain = niceDomain(initialBalance, max + padding);
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
        <defs>
          <Glow id={`glow-${uid}`} />
          <linearGradient id={`globalGrad-${uid}`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={color} stopOpacity={0.55} />
            <stop offset="55%" stopColor={color} stopOpacity={0.16} />
            <stop offset="100%" stopColor={color} stopOpacity={0.02} />
          </linearGradient>
          <linearGradient id={`globalLiveGrad-${uid}`} x1="0" y1="0" x2="0" y2="1">
        ...
        <Tooltip cursor={lineCursor} contentStyle={glassTooltip} labelStyle={tooltipLabelStyle} itemStyle={tooltipItemStyle}
          formatter={(value: number, name: string) => [
            ...
            name === 'balanceLive' ? 'Balance (Live 🔴)' : 'Global Balance',
          ]}
        />
        <Area type="linear" dataKey="balanceClosed" stroke={color} fill={`url(#globalGrad-${uid})`} strokeWidth={2.5} filter={`url(#glow-${uid})`}
          dot={hasLive ? false : makePulseDot(data.length - 1, color)}
          activeDot={{ r: 4, fill: color, stroke: color, strokeWidth: 2 }} connectNulls={false} />
        {hasLive && (
          <Area type="linear" dataKey="balanceLive" stroke={live} fill={`url(#globalLiveGrad-${uid})`} strokeWidth={2.5} strokeDasharray="6 3" filter={`url(#glow-${uid})`}
...
export default GlobalEquityCurveChart;
