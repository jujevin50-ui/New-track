    ...
    <SidebarProvider>
      <div className="min-h-screen flex w-full">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <header className="relative h-14 flex items-center gap-2 px-3 md:px-5 shrink-0 bg-background sticky top-0 z-30">
                ...
                <div className="hidden md:flex items-center gap-1">
                  {FILTERS.filter(f => f.key === 'all' || accounts.some(a => a.status === f.key)).map(f => (
                    <button
                      ...
                      className={`h-6 px-2 text-[10px] font-medium rounded-md transition-colors flex items-center gap-1 ${
                        statusFilter === f.key
                ...
                <div className="relative" ref={wrapperRef}>
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className="h-9 w-[180px] md:w-[220px] pl-8 text-xs bg-card/60 border-border/60 rounded-lg"
                    />
                  </div>
                  {dropdownOpen && (
                    <div className="absolute top-full mt-1 right-0 w-[300px] max-h-[320px] overflow-auto rounded-lg border border-border bg-popover shadow-lg z-50">
          ...
          <main key={location.pathname} className="flex-1 overflow-auto p-4 md:p-6 animate-page-in">
            {children}
          </main>
        </div>
      </div>
